...
class Esagrams extends AbstractService_1.default {
    static init(bot) {
        super.init(bot);
    ...
    static esagramExtraction(bot) {
        bot.onText(/\/esagramma/, (msg) => {
            let PImage = require('pureimage'), yImage = 400, xImage = 400, img1 = PImage.make(xImage, yImage), ctx = img1.getContext('2d'), startX = 0, endX = 400;
            ...
            for (let lines = 1; lines <= 6; lines++) {
                let effectiveY = xMultiplier * lines, number = Math.round((Math.random() * (9 - 6) + 6)), coin = number % 2 !== 0;
                ...
                for (let i = 0; i < thick; i++) {
                    if (!coin) {
                        ...
                        let partialOneEnd = endX / 2 - endX / 8, partialTwoStart = endX / 2 + endX / 8;
